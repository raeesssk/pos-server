...
router.post('/add', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    var singleInsert = 'INSERT INTO dealer_master(dm_firm_name,dm_number,dm_address,dm_credit, dm_opening_credit, dm_debit, dm_opening_debit, dm_srm_id, dm_status) values($1,$2,$3,$4,$5,$6,$7,$8,0) RETURNING *',
        params = [req.body.dm_firm_name,req.body.dm_number,req.body.dm_address,req.body.dm_credit,req.body.dm_opening_credit,req.body.dm_debit,req.body.dm_opening_debit,req.body.dm_srm_id]
...
router.post('/dealer/total', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    const strqry =  "SELECT count(dm_id) as total "+
                    ...
                    "inner join restaurant_master srm on dm.dm_srm_id=srm.srm_id "+
                    "where dm_status=0 "+
                    "and dm_srm_id = $1 "+
                    "and LOWER(dm_firm_name||''||dm_number||''||dm_address||''||dm_debit||''||dm_credit) LIKE LOWER($2);";

    const query = client.query(strqry,[req.body.dm_srm_id,str]);
    query.on('row', (row) => {
      results.push(row);
    });
    query.on('end', () => {
...
router.post('/dealer/limit', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    const strqry =  "SELECT * "+
                    ...
                    "and dm_srm_id = $1 "+
                    "and LOWER(dm_firm_name||''||dm_number||''||dm_address||''||dm_debit||''||dm_credit) LIKE LOWER($2) "+
                    "order by dm.dm_id desc LIMIT $3 OFFSET $4";

    const query = client.query(strqry,[req.body.dm_srm_id, str, req.body.number, req.body.begin]);
    query.on('row', (row) => {
      results.push(row);
    });
    query.on('end', () => {
