...
router.post('/pending', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    const query = client.query("SELECT * FROM order_product_master opm INNER JOIN order_master om on opm.opm_om_id = om.om_id INNER JOIN product_master pm on opm.opm_pm_id=pm.pm_id LEFT OUTER JOIN table_master tm on om.om_tm_id=tm.tm_id LEFT OUTER JOIN area_master am on tm.tm_am_id=am.am_id left outer join restaurant_master srm on pm.pm_srm_id=srm.srm_id where opm.opm_status_type='pending' order by opm.opm_id desc");
   ...
   client.query('BEGIN;');
    var singleInsert = "UPDATE order_product_master SET opm_status_type='completed' WHERE opm_id=$1 RETURNING *",
      ...
      client.query(singleInsert, params, function (error, result) {
        ...
        const query = client.query("SELECT * FROM recipe_master rm INNER JOIN product_master pm on rm.rm_pm_id=pm.pm_id left outer join restaurant_master srm on pm.pm_srm_id=srm.srm_id where pm_id=$1",[req.body.pm_id]);
        query.on('row', (row) => {
